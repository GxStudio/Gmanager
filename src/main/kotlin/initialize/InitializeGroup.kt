...
object InitializeGroup {
    ...
    fun main(group: Group) {
        ...
        if ((group.id in groupConfigList).not()) groupConfigList[group.id]=GroupConfig(group.id)
        if ((group.id in groupDataList).not()) groupDataList[group.id]= GroupData(group.id)
