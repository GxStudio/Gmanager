...
object Log {
    ...
    /** 细节*/
    fun v(msg: Any, title: String? = null): Log {
        if (level <= VERBOSE) MiraiLog.verbose(cw(msg, title))
        return this
    }
    
    /** 调试*/
    fun d(msg: Any, title: String? = null): Log {
        if (level <= DEBUG) MiraiLog.debug(cw(msg, title))
        return this
    }
    
    /** 信息*/
    fun i(msg: Any, title: String? = null): Log {
        if (level <= INFO) MiraiLog.info(cw(msg, title))
        return this
    }
    
    /** 警告*/
    fun w(msg: Any, title: String? = null): Log {
        if (level <= WARN) MiraiLog.warning(cw(msg, title))
        return this
    }
    
    /** 报错*/
    fun e(msg: Any, title: String? = null): Log {
        if (level <= ERROR) MiraiLog.error(cw(msg, title))
        return this
    }
    
    private fun cw(msg: Any, title: String?): String = if (title == null) msg.toString() else "[$title]$msg"
